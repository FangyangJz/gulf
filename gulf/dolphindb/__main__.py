...
from gulf.dolphindb.config.dimension_table import DimensionTable, StockBasicTable, BondBasicTable, TradeCalenderTable, \
    IndexHS300MembersTable, BondDelistTable, IndexHS300DailyTable, BondIndexTable, BondRedeemTable
from gulf.dolphindb.config.partition_table import PartitionTable, stock_daily_table, hk_hold_table, bond_daily_table, \
...
class Dolphindb:
    ...
    def __init__(
        ...
        self.stock_start_year = 2005  # 用于 数据库分区 和 确定数据下载范围
        self.bond_start_year = 2008  # 用于 数据库分区 和 确定数据下载范围
        self.end_year = 2030
        self.start_month = 1
        self.start_datetime = datetime.datetime(year=self.stock_start_year, month=self.start_month, day=1)
        ...
        self.init_db_path_dict = {
            ...
            DfsDbPath.btc_lob: self.init_btc_lob_db,
        }
    ...
    def init_stock_daily_code_db(self):
        # 2005 ~ 2030
        create_db_script = f"""
    ...
    def init_btc_lob_db(self):
        ...
        db=database('{DfsDbPath.btc_lob}', COMPO, [db_date_value, db_code_hash], engine=`{self.engine.value})
        """
